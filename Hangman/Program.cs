...
namespace Hangman
{
    class Program
    {
        ...
        {
            _words = File.ReadAllLines(Path.Join(Environment.CurrentDirectory, "words.txt")).ToArray();
            Game();
        }
        ...
        static char GetGuessFromUser(List<char> previous)
        {
            ...
            if (string.IsNullOrWhiteSpace(read) || read.Length > 1 ||!AllowedLetters.ContainsKey(read.ToLower().First()))
            {
